  ...
  <draggable
      ...
      >
       ...
       <div v-for="element in list"
          :key="element.item" class="flex">
          <Draggables @pinned='pinItem(element.item, $event)' v-bind:dragAnim="dragAnim" v-bind:username="user.username" v-bind:photo_url="user.photo_url" v-if="element.item=='Draggables'"/>
          <Stats @pinned='pinItem(element.item, !element.fixed)' :class="{'shake' : animated}" v-bind:pinned_class="element.fixed" v-else-if="element.item=='Stats'"/>
        </div>
        </transition-group>
    </draggable>
 </div>
...
export default {
  ...
  data () {
    return {
      ...
      user: {
        username: 'User',
        photo_url: 'https://api-private.atlassian.com/users/756e85b64322972fbe6f40fa056ae878/avatar'
      }
    }
  },
  created () {
